...
def _load_slo_metrics():
    ...
    incomplete, in which case the SLO-aware keys are simply absent rather than
...
def compute_metrics(
    ...
    metrics = {
        ...
        # Survivor-only: dropping a slow request improves these. See the
        # SLO-aware keys below for the all-requests view.
    ...
    # SLO-aware metrics over EVERY submitted request. TTFT here is
    # arrival -> first token (queue_wait_ms + ttft_ms), matching the survivor
    # percentiles above, converted ms -> s for the shared definition.
    _slo = _load_slo_metrics()
    if _slo is not None:
        samples = [
            (
                bool(r.get("success", False)),
                (r.get("queue_wait_ms", 0) + r.get("ttft_ms", 0)) / 1000.0,
                r.get("tokens_generated", 0),
            )
            for r in results
        ]
        metrics.update(_slo.slo_metrics(samples, duration_s))
        metrics.update(_slo.ttft_percentiles_all(samples))
    return metrics
...
def format_report(metrics: Dict[str, Any], config: Optional[Dict] = None,
    ...
    if "slo_attainment" in metrics:
        lines.append(f"  SLO attain:  {metrics['slo_attainment']*100:.1f}% "
                     f"({metrics['slo_met_requests']}/{metrics['total_requests']} "
                     f"within {metrics['slo_s']:.0f}s)")
        lines.append(f"  Effective:   {metrics['effective_throughput_tok_s']:.1f} tok/s "
                     f"(SLO-meeting requests only)")
        if fields == "all" and "ttft_all_p90_ms" in metrics:
            lines.append(f"  TTFT (all):  P50: {metrics['ttft_all_p50_ms']:.0f} ms   "
                         f"P90: {metrics['ttft_all_p90_ms']:.0f} ms")
