...
# Guarded post-admit margin, shared with SwarmLoRA and ServerlessLLM so the
# guards cannot drift apart.
def _load_margin():
    import importlib.util
    _p = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__)))), "src", "controller", "admission.py")
...
class BatchedWorker:
    ...
    def __init__(self, worker_id: int = 0, server_host: str = "localhost",
        ...
        # EMA of the work still owed after the deadline check passes: from
        # acquiring the model lock to the first token (tokenize + prefill).
        # Reserving it lets the check ask whether the deadline will pass before
        # the request finishes. Starts at 0 until the worker has observations.
    ...
    def _process_batch(self, batch: Batch) -> List[Dict]:
        ...
        try:
            ...
            with self._model_lock:
                # Enforce the request's deadline here as well: the controller's
                # budget covers the wait for a container, this covers the wait
                # for the model lock behind other batches. Requests past their
                # deadline are dropped before generation so the GPU serves ones
                # that can still answer in time. Dropped requests still get a
                # result entry so the reply stays 1:1 with what was sent.
                _t_admit = time.time()   # for the post-admit margin EMA
                _orig_requests = list(batch.requests)
                _expired = {}
                if any(getattr(r, "slo_deadline_wall", None) is not None
                       for r in _orig_requests):
                    _now = time.time()
                    # Reserve the work still owed after this check, so we do not
                    # admit a request that can only finish past its deadline.
                    _margin = _guarded_margin(self._post_admit_s,
                                              self._post_admit_n, _SLO_S)
                    _live = []
                    for _i, _r in enumerate(_orig_requests):
                        _dl = getattr(_r, "slo_deadline_wall", None)
                        if _dl is not None and (_now + _margin) > _dl:
                            _expired[_i] = {
                                "success": False,
                                "error": (f"SLO deadline passed while queued at "
                                          f"worker ({_now - _dl:.1f}s over)"),
                    ...
                    if not _live:
                        log(self.worker_id,
                            f"Batch {batch.batch_id}: all {len(_orig_requests)} "
                            f"requests past SLO deadline, skipping generation")
                        return [_expired[i] for i in range(len(_orig_requests))]
                    if _expired:
                        log(self.worker_id,
                            f"Batch {batch.batch_id}: dropped {len(_expired)}/"
                            f"{len(_orig_requests)} past SLO deadline")
                        batch.requests = _live
                        prompts = batch.prompts
                        batch_size = batch.size
            ...
            # Update the post-admit margin: lock acquired -> first token.
            if ttft_timer.t_first_token:
                _obs = max(0.0, ttft_timer.t_first_token - _t_admit)
                self._post_admit_s = (_obs if self._post_admit_n == 0
            ...
            # Re-interleave dropped requests so the reply is positionally 1:1
            # with the batch the controller sent.
            if _expired:
                merged, live_iter = [], iter(results)
                for _i in range(len(_orig_requests)):
                    merged.append(_expired[_i] if _i in _expired else next(live_iter))
                batch.requests = _orig_requests
                return merged
            ...
            traceback.print_exc()
            # Size to the original batch: pruning may have shrunk batch.requests.
            _n = len(locals().get("_orig_requests") or batch.requests)
            return [{"success": False, "error": str(e)} for _ in range(_n)]
    ...
    def start_http_server(self, http_port: int):
        ...
        def batch_execute():
            ...
            # Absolute per-request SLO deadlines, so this second queueing
            # stage is bounded by the same budget as the wait for a container.
            slo_deadlines = data.get('slo_deadlines', [])
            ...
            for i, p in enumerate(prompts):
                ...
                if i < len(slo_deadlines):
                    req.slo_deadline_wall = slo_deadlines[i]
