...
# Queue admission policy. Loaded from the repo's single canonical copy rather
# than vendored here: the fairness of the comparison rests on every system
# running the *same* policy, and a duplicated file would be free to drift.
# Loaded by path because this baseline has its own top-level `controller`
# module, so `import controller.admission` would resolve to the wrong one.
def _load_admission():
...
class Controller:
    ...
    def __init__(
        ...
        # and again inside the worker via the per-request slo_deadlines carried
        # to /batch_execute.
        self._admission = _admission_mod.AdmissionConfig.from_env()

        # E1: measured work remaining after a container is claimed (worker-side
        # queue + prefill). Drives the dynamic stage-1 deadline.
        self._e1_s = 0.0
        self._e1_n = 0
    ...
    def _dispatch_batch(self, batch: Batch) -> List[Dict]:
        ...
        if not container:
            reason = f"{queue_wait_ms:.0f}ms"
            logger.info(f"[TRACE] {batch.batch_id} STEP2_SELECT_FAIL fn={function_id} "
                       f"queue_wait_ms={queue_wait_ms:.0f} reason={reason}")
            return [{"success": False, "error": f"Queue timeout ({reason})",
        ...
        try:
            response = requests.post(
                ...
                json={
                    ...
                    # Per-request absolute deadline, carried into the worker so
                    # the wait inside the container (model lock, batches ahead)
                    # is bounded by the same budget as the wait for one.
                    "slo_deadlines": [
                        r.arrival_time_wall + self._admission.slo_s
            ...
            if response.status_code == 200:
                ...
                # Feed E1: worker-side ttft_ms is measured from dispatch, so it
                # is exactly the work owed after a container is claimed.
                if results:
                    _obs = results[0].get("ttft_ms", 0) / 1000.0
    ...
    def _select_and_claim(self, function_id: str, batch_size: int, arrival_wall=None):
        ...
        """
        # The request has until its latency target to obtain a container.
        slo_budget_s = self._admission.slo_s
        start_time = time.time()
        # Deadline is anchored to ARRIVAL (not now) so total queue wait is
        # bounded from arrival; queue_wait_ms below is left measured from
        # method entry to keep the reported metric unchanged (only the timeout
        # behaviour changes).
        deadline = (arrival_wall if arrival_wall is not None else start_time) + slo_budget_s
        spawn_triggered = False

        while True:
            remaining = deadline - time.time()
            # Dynamic deadline: reserve the work still owed after a container
            # is claimed, so we do not claim (and possibly swap) one for a
            # request that can no longer finish in time.
            _e1 = _admission_mod.post_admit_margin(
                self._e1_s, self._e1_n, self._admission.slo_s)
            if (time.time() + _e1) > deadline:
