...
# If no ordered traces found (e.g. poisson), sort numerically by rps --
# lexicographic order runs rps12 before rps2 and breaks the goodput baseline.
if [ ${#TRACES[@]} -eq 0 ]; then
...
# Set from the first trace's avg_completion_tokens and passed to later traces
# as --goodput-baseline-tokens, anchoring goodput to one low-load reference.
# Empty for the first trace, which falls back to self-baseline.
GOODPUT_BASELINE=""
LAST_OUTPUT_FILE=""
...
run_trace() {
    ...
    [[ -n "$GOODPUT_BASELINE" ]] && echo "  Throughput baseline: ${GOODPUT_BASELINE} tok/req (from first trace)"
    echo "========================================================"

    local goodput_args=()
    [[ -n "$GOODPUT_BASELINE" ]] && goodput_args=(--goodput-baseline-tokens "$GOODPUT_BASELINE")

    # RQ1 (3min Poisson sweep) only needs throughput/goodput for Fig. 5;
...
for trace_name in "${TRACES[@]}"; do
    ...
    if [[ -z "$GOODPUT_BASELINE" ]]; then
        GOODPUT_BASELINE=$(python3 -c "
import json
print(json.load(open('$LAST_OUTPUT_FILE'))['summary']['avg_completion_tokens'])
")
        echo "[baseline] Throughput reference set from $trace_name: ${GOODPUT_BASELINE} tok/req"
    fi
