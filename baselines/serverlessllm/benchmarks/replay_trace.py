...
def parse_args():
    ...
    parser.add_argument(
        ...
        default="all",
        help="Report detail: 'all' (acceptance/throughput/TTFT/TPOT, "
...
def _load_slo_metrics():
    """Load the repo's canonical SLO-metric definitions.

    Loaded by path, not vendored: all three systems must score against one
    definition or the comparison means nothing. Returns None if the tree is
    incomplete, so the SLO-aware keys are simply absent rather than the whole
    report failing.
    """
    ...
    path = os.path.join(
        ...
        "src", "controller", "slo_metrics.py",
    )
    try:
        spec = importlib.util.spec_from_file_location("swarm_slo_metrics", path)
...
def compute_summary(results, trace_file, gen_tok_per_sec=17.7):
    ...
    # SLO-aware metrics over EVERY submitted request, so drops and late
    # answers count against them. ttfts[] is built by iterating `successful`
    # in order and appends in both the measured and fallback branches, so it
    # aligns 1:1 with `successful`; zipping reuses that computation rather
    # than re-deriving TTFT and risking a second, divergent definition.
    # Values are already in SECONDS, which is what slo_metrics expects.
    _slo = _load_slo_metrics()
    if _slo is not None and len(ttfts) == len(successful):
        samples = [
            (True, t, r.completion_tokens) for r, t in zip(successful, ttfts)
        ] + [(False, 0.0, 0) for _ in failed]
        summary.update(_slo.slo_metrics(samples, total_duration))
        summary.update(_slo.ttft_percentiles_all(samples))
...
def print_report(summary, fields="all"):
    ...
    reads tokens_per_second -- TTFT/TPOT dropped since nothing consumes them
    here).
    """
    print()
    print("=" * 80)
    ...
    print(f"    Throughput:        {summary['tokens_per_second']:.1f} tok/s")
    if fields == "all":
    ...
    if "slo_attainment" in summary:
        print(f"    SLO attainment:    {summary['slo_attainment']*100:.1f}% "
              f"({summary['slo_met_requests']}/{summary['total_requests']} "
              f"within {summary['slo_s']:.0f}s)")
        print(f"    Effective tput:    {summary['effective_throughput_tok_s']:.1f} tok/s "
              f"(SLO-meeting requests only)")
        if fields == "all" and "ttft_all_p90_ms" in summary:
            print(f"    TTFT (all reqs):   P50: {summary['ttft_all_p50_ms']:.0f} ms   "
                  f"P90: {summary['ttft_all_p90_ms']:.0f} ms")
    print("=" * 80)
