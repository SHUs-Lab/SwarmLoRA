...
def best_effort_load(
    ...
    # _compute_device_placement_from_map_fast() is typed to return
    # Dict[str, Union[int, torch.device]] -- normalize to plain ints here,
    # once, since allocate_cuda_memory() and the device_uuid_map lookups
    # below both require int device indices.
    expanded_device_map = {
        name: (device.index if isinstance(device, torch.device) else device)
        for name, device in expanded_device_map.items()
    }
...
def load_lora(
    ...
    if incompatible_keys is not None:
        ...
        def _summarize_keys(keys, limit=3):
            # Key lists can run into the hundreds; summarize instead of
            # flooding the console.
            sample = ", ".join(keys[:limit])
            more = f" (+{len(keys) - limit} more)" if len(keys) > limit else ""
            return f"{len(keys)} key(s): {sample}{more}"
        ...
        if (
            ...
            err_msg = (
                ...
                f"unexpected keys not found in the model: "
                f"{_summarize_keys(incompatible_keys.unexpected_keys)}. "
            )
        ...
        if missing_keys:
            ...
            if lora_missing_keys:
                err_msg += (
                    f"Loading adapter weights from {origin_name} led to "
                    f"missing keys in the model: "
                    f"{_summarize_keys(lora_missing_keys)}"
                )

        if err_msg:
            logger.warning(err_msg)

    # `model` is already a PeftModel by this point (add_adapter/get_peft_model
    # above), and set_peft_model_state_dict already loaded this adapter's
    # weights into it -- re-wrapping here added one more base_model.model.
    # nesting layer per adapter loaded on this instance (visible as growing
    # key-name depth, and increasingly mismatched incompatible_keys above).
    peft_model = model
