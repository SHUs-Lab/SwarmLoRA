...
def create_app() -> FastAPI:
    ...
    async def register_handler(request: Request):
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Cannot register model: {type(e).__name__}: {e}",
            )
    ...
    async def inference_handler(request: Request, action: str):
        ...
        try:
            ...
            # Return the failure reason as JSON, matching the {"error": ...}
            # shape the router uses for its own timeouts, so clients can read
            # why a request was dropped. RayTaskError embeds the full remote
            # traceback, so take the final line -- the original message.
            text = str(e).strip()
            reason = text.splitlines()[-1] if text else repr(e)
            logger.info(f"Request failed for {model_name}: {reason}")
            return {"error": reason}
