...
def benchmark_disk_bandwidth(num_iterations=5):
    ...
    read_results = []
    try:
        # Unique per-call path -- this runs as a parallel Ray task on every
        # node, and a shared fixed filename lets one task's os.remove() race
        # another's open(), intermittently raising FileNotFoundError here.
        fd, temp_file = tempfile.mkstemp(prefix="disk_bandwidth_test_")
        os.close(fd)
