...
def download_lora_adapter(
    ...
    storage_path = os.getenv("STORAGE_PATH", "./models")
    # Use adapter_name (not adapter_name_or_path) for storage to avoid
    # absolute path conflicts on single-node setups
    adapter_path = os.path.join(
        storage_path, "transformers", adapter_name
    )
    ...
    try:
        model = PeftModel.from_pretrained(
            base_model, adapter_name_or_path, local_files_only=True
        )
    except Exception as e:
