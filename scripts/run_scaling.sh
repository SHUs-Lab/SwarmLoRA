...
# Sweep may exceed MAX_WORKERS_PER_GPU, so size the slot pool to the largest N
# (mirrors run_overhead.sh:19; without this the aggregator asserts on slot
# ownership once worker index passes the default pool size).
