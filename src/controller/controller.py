...
class AsyncLoadBalancer:
    ...
    async def route_request(self, req: InferenceRequest, arrival_wall_override: float = None) -> dict:
        ...
        if decision.action == RoutingAction.QUEUE:
            ...
            worker = await self._wait_for_available_worker(adapter_id)
        ...
        if not result.get('success') and 'Connect call failed' in result.get('error', ''):
            ...
            await self._remove_worker(worker.worker_id)
            retry_worker = await self._wait_for_available_worker(adapter_id)
            if retry_worker:
    ...
    async def _wait_for_available_worker(self, adapter_id: str) -> Optional[WorkerInfo]:
        """Wait for a worker to become available via centralized dispatch."""
        loop = asyncio.get_running_loop()
        future = loop.create_future()
        deadline = time.time() + self.scheduler_config.queue_timeout
        ...
        try:
            worker = await asyncio.wait_for(future, timeout=self.scheduler_config.queue_timeout)
            return worker
    ...
    async def _dispatch_loop(self):
        ...
        while self._running:
            ...
            while self._pending_requests:
                ...
                if pr.future.done() or pr.future.cancelled() or now > pr.deadline:
            ...
            for pr in self._pending_requests:
                if pr.future.done() or pr.future.cancelled() or time.time() > pr.deadline:
                    if not pr.future.done():
                        pr.future.cancel()
                    continue
...
if __name__ == "__main__":
    ...
    parser.add_argument("--queue-timeout", type=float, default=300.0,
                        help="Max seconds a request waits for a worker before being dropped (default: 300)")
    ...
    scheduler_config.queue_timeout = args.queue_timeout
