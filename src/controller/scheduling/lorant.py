...
class LoRantScheduler(LocalScheduler):
    ...
    def __init__(
        ...
        # Requests dropped recently for want of a worker, set by the
        # controller. A timed-out request leaves _pending_requests, so counting
        # drops keeps demand the pool failed to serve visible to the scaler.
        self._recent_drops: int = 0
    ...
    def _compute_demand_targets(
        ...
        # Pool size = busy workers + queued demand + demand that timed out,
        # with headroom. Busy alone grows logarithmically (small pool → small
        # target); pending_depth adds requests still waiting; recent_drops adds
        # those that gave up. Drops are ~0 in steady state.
        effective_demand = (busy_workers
                            + getattr(self, '_pending_depth', 0)
                            + getattr(self, '_recent_drops', 0))
        pool_size = min(
