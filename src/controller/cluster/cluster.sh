...
do_start() {
  ...
  if [[ -f "$PROJECT_ROOT/setup_mps.sh" ]]; then
    bash "$PROJECT_ROOT/setup_mps.sh" stop 2>/dev/null || true
    sleep 1
  ...
  for entry in "${NODES[@]}"; do
    ...
    if [[ "$HOST" == "local" ]]; then
      ...
      ssh -p "$ssh_port" -o ConnectTimeout=10 -o StrictHostKeyChecking=no -o BatchMode=yes -f "$HOST" \
        "export LD_LIBRARY_PATH=$remote_root/venv/lib/python3.12/site-packages/torch/lib:\$LD_LIBRARY_PATH; \
