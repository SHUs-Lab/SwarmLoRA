...
class NodeAgent:
    ...
    async def _launch_controller(self):
        ...
        logger.info("AGENT DEVICE PLAN: detected=%d devices=%s per_gpu_cap=%s max_workers=%s",
                    num_gpus,
                    ",".join(f"cuda:{g['index']}" for g in self._detected_gpus) or "cuda:0",
                    self._per_gpu_cap(num_gpus), self.config.max_workers)
