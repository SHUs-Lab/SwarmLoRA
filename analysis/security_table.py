...
def _weight_theft(attack):
    """Direct (positional) cosine of the stolen LoRA-A block vs. the victim's
    known weights (~1.0 = the exfiltrated bytes are the victim's weights).
    Falls back to the recovery flag when nothing was read."""
    wrs = attack.get("data_theft", {}).get("wrs_auth", {})
    cos = wrs.get("victim_weight_cosine_sim_direct")
    if cos is not None:
        return f"{cos:.2f}"
