...
def serverlessllm_series():
    ...
    Reports goodput, not raw tokens/s (paper convention, ServerlessLLM only):
    its admission-queue timeout preferentially completes long requests under
    load, inflating raw throughput. The other two systems use raw tokens/s.
    ...
    for n in RATES:
        ...
        if files:
            ...
            out[n] = s.get("goodput_tok_per_s", s.get("tokens_per_second"))
    return out if len(out) == len(RATES) else None
...
def main():
    ...
    for name, series in data.items():
        tag = " (goodput)" if name == "ServerlessLLM" else ""
