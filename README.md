...
NVIDIA L40S (cluster), Llama-3.1-8B-Instruct.
...
**Left:** SwarmLoRA achieves **954 tok/s at 8 RPS** — 4.1× higher than ServerlessLoRA, 4.9× higher than ServerlessLLM.
**Right:** Worker spawn in **618 ms**, adapter swap in **121 ms** (5.1× faster).

### IPC overhead breakdown

<p align="center">
<img src="assets/overhead_breakdown.png" width="48%">
<img src="assets/overhead_throughput.png" width="48%">
</p>

**Left:** the split architecture adds **20.2% overhead** at N=1 (9.3 ms over
the 45.9 ms HF+PEFT baseline), and IPC's share of per-token cost falls as
workers are added.
**Right:** at N=28 concurrent workers the system reaches **314 tok/s** — a
14.4× improvement over single-worker inference.

### Trace-driven evaluation

| Trace category | TPOT P50 | Throughput |
|----------------|----------|------------|
| Steady-Light | 51 ms | 204 tok/s |
| Bursty-Light | 56 ms | 249 tok/s |
| Normal | 54 ms | 324 tok/s |
| Steady-Heavy | 68 ms | 977 tok/s |
| Bursty-Heavy | 68 ms | 882 tok/s |

### Adapter pool scalability

| Pool size | TTFT P50 | TPOT P50 | Swap Rate |
|-----------|----------|----------|-----------|
| 10 adapters | 150 ms | 57.5 ms | 53% |
| 100 adapters | 150 ms | 58.7 ms | 88% |
| 500 adapters | 150 ms | 58.4 ms | 115% |

Both TTFT and TPOT stay flat across pool sizes even as the swap rate
more than doubles — the base-model GEMM cost is independent of adapter
count. Pool size shows up in admission instead: acceptance falls from
99.8% to 94.8% and throughput from 837 to 754 tok/s over the same range.
...
- `bash scripts/run_scalability.sh all` — adapter pool scalability, 10 to 500 adapters (Fig. 13; needs a 2-node cluster; see `src/controller/cluster/cluster.conf.example`)
